...
public class LineChartRenderer extends LineRadarRenderer {
    ...
    private void drawLinearDisConnect(ILineDataSet dataSet) {
...
//                YAxis axis = mChart.getAxis(YAxis.AxisDependency.LEFT);
//                formattedValue = axis.getValueFormatter().getFormattedValue(j, axis);
//                if ("-127分" == prevValue) {
//                    cubicPath.moveTo(cur.getX(), cur.getY() * phaseY);
//                    continue;
//                }
//                if ("-127分" == formattedValue) {
//                    continue;
//                }
                if (prev.getY() < -120) {
                    cubicPath.moveTo(cur.getX(), cur.getY() * phaseY);
    ...
    public void drawValues(Canvas c) {
        ...
        if (isDrawingValuesAllowed(mChart)) {
            ...
            for (int i = 0; i < dataSets.size(); i++) {
                ...
                //曲线和字的间距
                int valOffset = (int) (dataSet.getCircleRadius() * 2.5f);
